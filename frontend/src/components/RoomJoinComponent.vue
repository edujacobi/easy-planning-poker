		...
		<GlassCard>
			<div class="grid grid-cols-2 gap-1 px-3">
				<button @click="activeTab = 'join'"
					...
					:class="activeTab === 'join' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-400 hover:text-slate-200'">
					Join room
				</button>
				<button @click="activeTab = 'create'"
					class="py-2.5 px-4 rounded-xl text-sm font-medium transition-all duration-150"
					:class="activeTab === 'create' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-400 hover:text-slate-200'">
					Create room
				</button>
			</div>

			<!-- Join Room Pane -->
			<div v-if="activeTab === 'join'" class="p-4 space-y-4">
				<FormGroup label="Room code or ID" id="roomId">
					<FlexRow class="w-full">
						...
						<PrimaryButton @click="joinRoom" size="sm">
							Join
							<Play />
						</PrimaryButton>
					</FlexRow>
			...
			<!-- Create Room Pane -->
			<div v-if="activeTab === 'create'" class="px-4 py-2 space-y-4">

				<!-- Room Title -->
				<FormGroup label="Room title" id="roomTitle">
					<Input id="roomTitle" v-model="roomTitle" placeholder="Death Star team estimation" />
				</FormGroup>

				<!-- Deck selection -->
				<FlexCol gap="2">
					<Label>Choose vote style</Label>
					<div class="grid grid-cols-2 gap-3">
						<button type="button" @click="voteType = 'fibonacci'"
							...
							:class="voteType === 'fibonacci' ? 'border-indigo-500 ring-2 ring-indigo-500/10 bg-indigo-700/10 hover:bg-indigo-700/15' : 'border-slate-800 hover:border-slate-700'">
						...
						<button type="button" @click="voteType = 'linear'"
							class="p-4 rounded-xl border bg-slate-950/60 hover:bg-slate-950 text-left transition-all duration-150"
							:class="voteType === 'linear' ? 'border-indigo-500 ring-2 ring-indigo-500/10 bg-indigo-700/10 hover:bg-indigo-700/15' : 'border-slate-800 hover:border-slate-700'">
				...
				<FlexCol gap="3" class="py-2">
					...
					<div class="space-y-4 pr-1">
						<div v-for="(story, sIdx) in stories" :key="sIdx" class="space-y-3 relative group">
							<!-- Story Header -->
							<FlexRow>
								<div class="text-sm bg-slate-800 text-slate-300 px-3 py-0.5 rounded font-mono shrink-0">
									Story {{ sIdx + 1 }}
								</div>
							...
							<!-- Nested Tasks -->
							<div class="pl-6 space-y-2 border-l border-slate-800 pb-3">
								<FlexRow v-for="(_, tIdx) in story.tasks" :key="tIdx">
									<span class="text-sm text-slate-500 font-mono shrink-0">
										Task {{ tIdx + 1 }}
									</span>
								...
								<OutlineButton @click="addTask(sIdx)" size="sm">
