...
<template>
  <div class="fixed inset-0 bg-slate-950/80 backdrop-blur-md flex items-center justify-center z-50 p-4">
    <!-- Floating Alert -->
    <div v-if="alertMessage" class="fixed top-4 left-1/2 -translate-x-1/2 z-[60] w-full max-w-md px-4 alert-fade-in">
    ...
    <GlassCard class="w-full max-w-xl shadow-2xl max-h-[85vh] overflow-y-auto flex flex-col">
      <CardHeader class="border-b border-slate-800 pb-3">
        <CardTitle class="text-white text-xl">Add More Stories & Tasks</CardTitle>
      ...
      <CardContent class="flex-1 p-6 space-y-4">
        ...
        <div v-if="!isAdmin" class="text-center py-10 space-y-3">
          ...
          <p class="text-sm text-slate-400">Admin is currently adding more histories to estimation backlog...</p>
        </div>
            ...
            <div v-for="(story, sIdx) in newStories" :key="sIdx"
              ...
              <FlexRow>
                <Input v-model="story.title" placeholder="Story Title..."
                  class="bg-slate-950 border-slate-800 text-slate-100 text-sm h-8" />
              ...
              <div class="pl-4 space-y-2 border-l border-slate-800">
                <FlexRow v-for="(_, tIdx) in story.tasks" :key="tIdx">
                  <Input v-model="story.tasks[tIdx]" placeholder="Task Title..."
                    class="bg-slate-950 border-slate-800 text-slate-300 text-xs h-7" />
