...
const copyButtonText = computed(() =>
	isCopiedMd.value ? "RESULTS.md Copied!" : "Copy RESULTS.md File Content",
);
</script>

<template>
	<div
		class="fixed inset-0 bg-slate-950/90 backdrop-blur-lg flex items-center justify-center z-50 p-4 md:p-8"
	>
		<GlassCard class="w-full max-w-2xl">
			<CardHeader class="text-center">
				...
				<CardDescription>
					Here are the compiled statistics and results of the Planning
					Poker session.
				</CardDescription>
			</CardHeader>
			...
			<CardContent>
				...
				>
					<HighlightedValue
						title="Stories voted"
						:value="sessionFinishedData?.totalStories"
					/>
					<HighlightedValue
						title="Total points"
						:value="sessionFinishedData?.totalPoints"
					/>
					<HighlightedValue
						title="Participants"
						:value="sessionFinishedData?.participantsCount"
					/>
				</Grid>
				...
				<FormGroup
					id="markdownPreview"
					label="RESULTS.md Preview:"
				>
					<PreFormattedText
						:markdownContent="sessionFinishedData?.markdown"
					/>
				</FormGroup>
			</CardContent>
			...
			<FlexRow
				class="p-4 py-0 gap-3"
				justify="between"
			>
				<PrimaryButton
					class="flex-1 py-6 text-sm"
					@click="copyResultsMarkdown"
				>
