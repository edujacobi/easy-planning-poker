...
export class RoomsService {
  ...
  getOrCreateActiveState(roomId: string, defaultTaskId: string | null = null): ActiveRoomState {
  ...
  async addStories(roomId: string, stories: Array<{ title: string; tasks: string[] }>) {
    const existingStories = await this.storyModel.findAll({ where: { roomId } });
    ...
    if (!state.activeTaskId && addedStories.length > 0 && addedStories[0].tasks && addedStories[0].tasks.length > 0) {
  ...
  async joinParticipant(roomId: string, user: { userId: string; nickname: string; emoji: string }) {
  ...
  castVote(roomId: string, userId: string, voteValue: string): ActiveRoomState {
    const state = this.getOrCreateActiveState(roomId);
  ...
  async confirmPoints(roomId: string, points: number | null): Promise<ActiveRoomState> {
    const state = this.getOrCreateActiveState(roomId);
  ...
  private async findNextUnvotedTaskId(roomId: string): Promise<string | null> {
  ...
  async generateRoomResults(roomId: string) {
    ...
    const participants = await this.participantModel.findAll({ where: { roomId } });
    ...
    for (const story of room.stories) {
      ...
      for (const task of story.tasks) {
        const pointsLabel = task.points !== null ? task.points.toString() : 'Not Voted / Skipped';
