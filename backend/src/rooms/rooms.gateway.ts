...
export class RoomsGateway implements OnGatewayConnection, OnGatewayDisconnect {
	...
	async handleConnection(client: Socket) {
		...
		await this.roomsService.joinParticipant(roomId, {
			userId,
			nickname,
			emoji,
		});
	...
	async handleRevealVotes(@ConnectedSocket() client: Socket) {
		...
		if (state.activeTaskId) {
			const task = await this.roomsService
				.getRoomDetails(roomId)
				.then((r) => {
					for (const s of r.stories) {
						const t = s.tasks.find(
							(tk) => tk.id === state.activeTaskId,
						);
						if (t) return t;
					}
					return null;
	...
	async handleFinishSession(
		...
		if (data.decision === "add_stories") {
			this.server
				.to(roomId)
				.emit("sessionStateChange", { mode: "add_stories" });
		} else if (data.decision === "cancel") {
			this.server
				.to(roomId)
				.emit("sessionStateChange", { mode: "voting" });
	...
	async handleAddStories(
		@ConnectedSocket() client: Socket,
		@MessageBody()
		data: { stories: Array<{ title: string; tasks: string[] }> },
	) {
		const { roomId, userId } = client.handshake.query as {
			roomId: string;
			userId: string;
	...
	private async broadcastRoomState(roomId: string) {
		...
		const players = room.participants.map((p) => {
			...
			const voteValue = state.votingRevealed
				? state.votes[p.userId] || null
				: null;
	...
	private calculateStats(votes: Record<string, string>) {
		...
		if (numericVotes.length > 0) {
			mean =
				numericVotes.reduce((sum, v) => sum + v, 0) /
				numericVotes.length;
